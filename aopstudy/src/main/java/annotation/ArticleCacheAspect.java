...
/*
 * Order(1)AOP -> Order(2)AOP -> Order(3)AOP -> 필수메서드 -> 
 *   Order(3)AOP -> Order(2)AOP -> Order(1)AOP
 */
@Component //객체화
@Aspect    //AOP 클래스
@Order(2)  //2번째 순서. 실행전에는 순서대로, 실행 후는 역순으로 실행
public class ArticleCacheAspect {
	private Map<Integer,Article> cache=new HashMap<Integer,Article>();
	/*
	 * execution(public * *..ReadArticleService.*(..)) : 이름이 ReadArticleService클래스의 모든 public 메서드
	 *   public : public 메서드
	 *   *      : 리턴타입에 상관없이.
	 *   *..    : 패키명에 상관없이
	 ...
	 *   *(..)  : 메서드의 매개변수 목록과 상관없이
	 *   
	 *   @Around : advice 중 필수메서드의 실행 전과 후 모두 
	 */
	@Around("execution(public * *..ReadArticleService.*(..))")
	public Object cache(ProceedingJoinPoint joinPoint) throws Throwable {
		//joinPoint : 실행 순서
		//joinPoint.getArgs() : 필수메서드 실행시 호출한 매개변수 값의 목록
		Integer id = (Integer)joinPoint.getArgs()[0];
		//joinPoint.getSignature().getName() : 필수메서드의 메서드명
		...
		//ret : 필수메서드의 리턴값
		Object ret = joinPoint.proceed(); //다음순서 메서드 실행. 필수메서드 실행됨.
		//필수메서드 실행 이후. 필수 메서드에서 예외 발생시 실행되지 않음.
